...
*** Background
Before we can create the index, a background must be created.
...
#+end_src
*** Index
After the background files are there, we can construct the dl3 index files.
Strictly speaking this could be done before as well as long as you do not read the files
before the background models are there, but this way around you can be sure dl3 is
done and working once the index is there.
