...
def DL3_IRF_PLOTS(wildcards):
    ...
    return [out / f"{irf}/{irf}_{run_id}.pdf" for irf in irfs_to_produce for run_id in ids]
...
rule dl4:
    input:
        [dl4 / f"{analysis}/plots/{plot}.pdf" for analysis in analyses for plot in dl4_plot_types]
...
dl5_plot_types = ["significance_map", "significance_distribution", "2d_flux_profile", "flux_points", "fit_residuals"]
#_curve] significance distribution on vs off

rule dl5:
    input:
        [dl5 / f"{analysis}/plots/{plot}.pdf" for analysis in analyses for plot in dl5_plot_types]
