...
** TODO
This is not very well separated from [[DL1]] unfortunately...
The problem is, that I need to do some extra things in here, because there
are no allsky test files.
Otherwise I would use them directly (and link them in the previous step) and then
also use the models, that exist for these files.
That is pretty much what we do for the 1D-case (because for pointlike gammas diffuse test files
actually exist!).

A semi-clean solution would be to handle all linking of mc-related studd in here.
Right now I link the nodes and model config(s) in [[DL1]] because of the way the scripts and
workflow developed. It is a TODO to change that.
...
I am not quite sure if I am happy with the /all_gamma_nodes/ solution.
Need to test it to make sure it works as expected.

In the merge rule there is a hardcoded GammaDiffuse, which I also dont like much.
Reason is simple: I dont need to merge protons, because we can use the "premerged" file since
I do not need test files for them. But maybe it would be nicer to still do the merging(?).
...
After that is done, the gamma test files can be linked to match the observed runs.
That is the IRF matching basically.
This step is not really necessary with lstchain 0.10 as then the matching happens in the
dl3 step and instead we would want an IRF for each mc node irrespective of the observed
runs. That will also simplify things a bit, so that should be something I do next.
...
rule link_test_nodes_to_dl1_runs:
    input:
        ...
        test_files=expand(
            mc / "GammaDiffuse/{node}_test.dl1.h5", node=all_gamma_nodes
        ),
    output:
        runs=expand(
...
The proton file is already merged, so I only need to merge the diffuse gammas here
although merging protons too would not be a big deal.
