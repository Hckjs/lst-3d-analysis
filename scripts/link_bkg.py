...
def main(hdu_index_path, bkg_dir, bkg_file):
    ...
    if len(bkg_file) == 1:
        ...
        print("Number of bkg files matching. Sorting them assuming that leads to the same order as the obs_ids")
        bkg_file = sorted(bkg_file)
    else:
        raise Exception(f"Number of bkg files does not match. We have {len(ids)} obs, but {len(bkg_file)} bkg files")
